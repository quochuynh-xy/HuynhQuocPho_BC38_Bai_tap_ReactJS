...
class Products extends Component {
  ...
  renderProducts () {
    let html = glasses.map((currValue, index)=> {
      return (
        <div className="col-2 text-center py-2" key={currValue.id}>
          <img onClick={()=>{this.tryThisGlass(index)}} className={`${css.single_item} w-100 p-2`} src={currValue.url} alt={currValue.id} />
        </div>
      )
    })
    return html
      ...
      <section className={`container pt-5 ${css.product}`}>
            ...
            <div className="row justify-content-evenly">
                <div className={`${css.product_des} col-3 px-0`}>
                    ...
                    <div className={`${css.description} ${this.state.show}`}>
                        <p className="mb-1 text-primary fw-bold">{this.state.glassName}</p>
                        <p className="text-white">{this.state.glassDesc}</p>
                    </div>
                    <img className={`${css.testProduct}`} src={this.state.imgSource} alt="" />
                </div>
        ...
        <div className="row">
          ...
          <div className="col-2 text-center py-2 d-flex justify-content-center">
            <button onClick={()=>{this.tryThisGlass("remove")}} className="btn btn-success px-3 my-2 mx-2 rounded-3">Tháo kính</button>
          </div>
        </div>
      </section>
    )
